...
function rscDrawFormCopy($im, array $row, int $left, int $top, int $width): int
{
    ...
    $m4 = $width - $m1 - $m2 - $m3;
    $cx = $x;
    foreach (['Last Name', 'First Name', 'Research 1 / Defense O.R. No.', 'Remarks'] as $i => $label) {
        $w = [$m1, $m2, $m3, $m4][$i];
    ...
    $misDate = rscFormatDate($row['mis_verified_at'] ?? $row['uploaded_at'] ?? $row['adviser_signed_at'] ?? null);
    $aaDate = rscFormatDate($row['aa_verified_at'] ?? $row['uploaded_at'] ?? $row['adviser_signed_at'] ?? null);
