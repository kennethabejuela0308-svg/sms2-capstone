...
function rscEnsureForReadyGroup(PDO $crad, int $groupId): ?array
{
    ...
    if (in_array((string) $existing['status'], ['draft', 'sent_to_adviser'], true)) {
        $crad->prepare(
...
{
    $members = [];
    $decoded = json_decode((string) ($row['members_json'] ?? ''), true);
    if (is_array($decoded)) {
        $members = $decoded;
    }
    return [
...
function rscRenderFormHtml(array $row, bool $duplicate = true): string
{
    $copy = static function (array $row): string {
        ...
        return '<div class="rsc-sheet">'
            ...
            . '<tr><td>2. OR no. Verified by Accounting</td><td>Accounting:</td><td></td></tr>'
            . '<tr><td>3. Turnitin username and Password Released by AAI</td><td>AAI:</td><td></td></tr>'
