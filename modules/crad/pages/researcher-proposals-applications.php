<?php
/**
 * SMS 2 - CORE SYSTEM · Proposals & Applications — Researcher View
 * Module: CRAD
 *
 * Researcher-facing page that lists ONLY the authenticated user's own research
 * grant proposals/applications (scoped by applicant_user_id in the database).
 *
 * Students see their own student proposals; Research Advisers (faculty
 * researchers) see their own personal grant proposals. Adviser-to-student
 * Research Monitoring relationships are intentionally NOT mixed in: a faculty
 * member's Proposals & Applications never includes proposals submitted by
 * students merely because the faculty member is their adviser.
 */
...
$pageBannerDescription = 'Your own research grant proposals and their evaluation status.';
...
if ($crad) {
    try {
        ...
        $applications  = grantGetUserApplications($crad, (int) ($_SESSION['user_id'] ?? 0));
        $opportunities = grantGetOpportunities($crad);
    } catch (Throwable $e) {
        $dbError = htmlspecialchars($e->getMessage());
        error_log('researcher-proposals-applications: ' . $e->getMessage());
    }
} else {
 ...
 */
function rpaStatusLabel(string $status): string
{
    return $status === 'Submitted' ? 'Pending Evaluation' : $status;
}

function rpaStatusBadge(string $status): string
{
    $label = rpaStatusLabel($status);
    $map   = [
...
<div class="mpl" data-mpl data-researcher-proposals-page>

    <!-- ── Top bar ──────────────────────────────────────────────────────── -->
    <div class="mpl-top">
        <p>Your own grant proposals submitted through the BRGFAMS Form 1 workflow. Only your records are shown.</p>
        <div class="mpl-toolbar">
            <a class="mpl-btn mpl-btn-soft"
               href="<?= BASE_URL ?>/modules/crad/pages/researcher-grant-opportunities.php">
    ...
    <section class="mpl-stats" aria-label="Proposal summary">
        <article class="mpl-stat">
            ...
            <div><span>My Proposals</span><strong><?= $total ?></strong></div>
        </article>
        <article class="mpl-stat">
    ...
    <div class="mpl-filters">
        <label class="mpl-search">
            ...
            <input type="search" id="paSearch"
                   placeholder="Search by grant, title, college, or status…"
    ...
    <section class="mpl-panel">
        <div class="mpl-panel-head">
            <div>
                <h2>My Grant Proposals</h2>
                <p>All grant proposals you submitted. Click a row to expand proposal details.</p>
            </div>
        </div>
            ...
            <table class="mpl-table" id="paTable">
                ...
                <tbody id="paTableBody">
                    <?php if (empty($applications)): ?>
                        <tr>
                            <td colspan="7" style="text-align:center;color:var(--sms-text-muted);padding:2.5rem;">
                                You have not submitted any grant proposals yet.
                                <br><a href="<?= BASE_URL ?>/modules/crad/pages/researcher-grant-opportunities.php"
                                       style="color:var(--sms-primary);">
                                    Browse Grant Opportunities to apply.
                                </a>
                            </td>
                        </tr>
                    <?php else: ?>
                        <?php foreach ($applications as $app):
                            $statusLabel  = rpaStatusLabel((string) $app['status']);
                        ...
                        <tr class="pa-row"
                            ...
                            <td><?= rpaStatusBadge((string) $app['status']) ?></td>
...
(function () {
    ...
    function filterTable() {
        ...
        dataRows.forEach(function (row) {
            ...
            var visible     = matchTerm && matchStatus;
