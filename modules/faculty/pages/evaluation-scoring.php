        ...
        <div class="row g-4">
            <div class="col-lg-4">
                <section class="glass-panel p-4 h-100">
                    ...
                    <hr>
                    <a class="btn btn-outline-primary w-100 mb-2" href="<?= e(chapterDocumentUrl((int) $submission['id'])) ?>" target="_blank"><i class="fas fa-eye me-2"></i>View/Open Document</a>
                        ...
                        <form method="post" data-once-form>
                            ...
                            <?php foreach ([['content','Content'],['methodology','Methodology'],['references','References'],['format','Format']] as [$key, $label]): ?>
                                <div class="row g-2 align-items-end mb-3">
                                    <div class="col-md-3"><label class="form-label"><?= e($label) ?> Score</label><input type="number" class="form-control" name="<?= e($key) ?>_score" min="0" max="100" step="0.01" required></div>
                                    <div class="col-md-9"><label class="form-label"><?= e($label) ?> Remarks</label><input type="text" class="form-control" name="<?= e($key) ?>_remarks" maxlength="1000"></div>
                                </div>
                            <?php endforeach; ?>
                            <div class="mb-3"><label class="form-label">General Evaluation Feedback</label><textarea class="form-control" name="overall_feedback" rows="4"></textarea></div>
                            <div class="mb-3"><label class="form-label">Result</label><select class="form-select" name="result" required><option value="">Select result...</option><option value="APPROVED">APPROVED</option><option value="APPROVED WITH REVISION">APPROVED WITH REVISION</option></select></div>
